...
if (!function_exists('product_list_elegant') && function_exists("is_woocommerce")) {
    function product_list_elegant($atts, $content = null) {
        ...
        $q = new WP_Query(
            array('post_type' => 'product', 'posts_per_page' => $per_page, 'category_name' => $category, 'orderby' => $order_by, 'order' => $order)
        );
        ...
        while ($q->have_posts()) : $q->the_post();
            ...
            $html .= '</div>';
            $html .= '<a class="product_list_link" href="'.get_the_permalink().'" target="_self"></a>';
            $html .= '</li>';
