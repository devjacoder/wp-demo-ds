...
*/
    ...
    function wpdev_bk_define_payment_forms($blank, $booking_id, $summ,$bk_title, $booking_days_count, $booking_type, $bkform, $wp_nonce, $is_deposit=false, $additional_calendars = array() ){
        ...
        $output = str_replace('\"','"',$output);
//debuge($_REQUEST , $booking_id, $bkform, $booking_type, $additional_calendars);die;
        ...
        if ($is_show_booking_summary_in_payment_form == 'On')
            ...
            // Auto  redirect  to the "Payment page",  instead of showing payment form  in Ajax request.
            // Need correcttly  to  configure [bookingedit] shortcode at "Edit/Payment" page.
            if (0)                                                              //FixIn: 6.2.2.2
                if ( ! isset( $_REQUEST['booking_hash'] ) ) {
                    ...
                    if ($my_booking_id_type !== false) {
                        ...
                        $url = get_bk_option( 'booking_url_bookings_edit_by_visitors');
                        $url = apply_bk_filter('wpdev_check_for_active_language', $url );
                        $url = rtrim($url, '/') . '/?booking_hash='. $my_edited_bk_hash .'&booking_pay=1';
...
//            $my_dates_4_send = change_date_format(get_dates_str($booking_id));
       ...
       /**/
        ...
        //FixIn:6.1.1.20
        $only_days = array();
        foreach ( $check_in_out as $d_value ) {
            $d_value = trim($d_value);
            $d_value = explode(' ', $d_value);
            $new_day = explode('-', $d_value[0]);
            $only_days[] = sprintf( "%02d.%02d.%04d", intval($new_day[2]), intval($new_day[1]), intval($new_day[0]) );                
        }
        $only_days = implode(',',$only_days);
        $dates_in_diff_formats = wpbc_get_dates_in_diff_formats( $only_days, $booking_type, $bkform  );
        $coupon_info_4_main_calendar = apply_bk_filter('wpdev_get_additional_description_about_coupons', '', $booking_type, $only_days, array($dates_in_diff_formats['start_time'], $dates_in_diff_formats['end_time'] ), $bkform   );
        ...
        $output = '<div  class="booking_summary" >';
            $output .= '<div  class="booking_summary_dates" style="margin:10px 0px;"><label class="booking_summary_dates_title" style="font-weight: 600;">' . __('Dates:' ,'booking') .'</label> ' . $my_dates_4_send . '</div>';
//            $output .= '<div  class="booking_summary_dates" style="margin:10px 0px;font-weight: 600;">' . __('Cost:' ,'booking') . ' ' . $cost_currency . ' ' . $my_cost . ' ' . $coupon_info_4_main_calendar . '</div>';
            $output .= '<div  class="booking_summary_data_title" style="font-weight: 600;" >' . __('Booking Details:' ,'booking') . '</div>';
