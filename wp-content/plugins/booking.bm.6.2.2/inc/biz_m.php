...
//        $summ_deposit = apply_bk_filter('advanced_cost_apply', $summ_deposit , $_POST['form'], $_POST['bk_type'], explode(',', $dates)  );    // Fix: 6.1.1.12
       ...
       function resources_settings_table_collumns( $bt, $all_id, $alternative_color, $advanced_params = array() ){
          ...
          if (isset($_GET['tab'])) if (  ($_GET['tab']=='cost')  ) {  $link .= '&tab=cost'; ?>
                ...
                <td style="text-align:center;border-left:1px solid #ccc;" <?php echo $alternative_color; ?> >
                    <a     class="button" style="font-size: 12px !important;" 
                    ...
                    <?php } else { ?>
                      <a   rel="tooltip" class="tooltip_top button disabled"  style="font-size: 12px !important;"
                    ...
                    <?php /*
                          ...
                          <?php /* } else { ?>
                            <a   style="font-size: 12px !important;"  rel="tooltip" class="tooltip_top button disabled" title="<?php echo __('Set the deposit payment required for the payment form' ,'booking'); ?>"
            ...
            <?php } if (isset($_GET['tab'])) if ( ($_GET['tab']=='availability')  ) {  $link .= '&tab=availability'; ?>
                    <td style="text-align:center;font-size: 12px;border-left:1px solid #ccc;" <?php echo $alternative_color; ?> >
...
//if ( (!empty ($starttimes['start_time'])) && (!empty ($starttimes['end_time'])) && ( get_bk_option( 'booking_paypal_price_period' ) !== 'hour') ) { continue; }
                                        ...
                                        ?>
                                              <tr>
                                                  ...
                                                  <td class="<?php echo $td_class , $is_checked; ?>" style="text-align: center;font-weight: 600;">
                                                    ...
                                                    <?php
                                                            if ($cost !== '') {
                                                                ...
                                                                echo ' <span style="font-weight: 400;">', get_bk_option( 'booking_paypal_curency' ); ?>                                                                          
                                                                          <?php if( get_bk_option( 'booking_paypal_price_period' ) == 'day')    _e('for 1 day' ,'booking');    ?>
                                                                          <?php if( get_bk_option( 'booking_paypal_price_period' ) == 'night')  _e('for 1 night' ,'booking');  ?>
                                                                          <?php if( get_bk_option( 'booking_paypal_price_period' ) == 'fixed')  _e('fixed deposit' ,'booking');?>
                                                                          <?php if( get_bk_option( 'booking_paypal_price_period' ) == 'hour')   _e('for 1 hour' ,'booking');   ?>
...
//debuge($days_costs);                                    
// FixIn:6.2.1.3                                return $days_costs;
...
// FixIn:6.2.1.3                             if ( isset( $days_costs[ $value['from'] ] ) ) continue; // Aleready set this option                                
...
<!--                        <div class="wpbc-help-message" style="font-size:1em;"><?php echo '<strong>'; _e('Note!' ,'booking'); echo '</strong> ';printf(__('If you add costs here, %srates%s for this booking resource will not take effect !!!' ,'booking'), '<a href="'. $link.'&wpdev_edit_rates=' . $edit_id .'">','</a>'); ?></div>-->
                ...
                <script type="text/javascript">
                    ...
                    function add_new_days_cost_row(){
                        ...
                        d_html += '               if ( this.options[this.selectedIndex].value != \'summ\' ) { addRowForCustomizationCostDependsFromNumSellDays('+row__id+'); } ';
                        d_html += '               else  { addRowForCustomizationCostDependsFromNumSellDays4Summ('+row__id+'); } " >';
    ...
    function form_for_range_days_filter($wpdev_edit_id){ global $wpdb;
        ...
        if (isset($_POST['filter_name_range_season_filter'])) {             //Only the form (verison 2.0) have this field, so we will save in current format
            ...
            if ( false === $wpdb->query( $sql ) ){
               echo '<div class="error_message ajax_message textleft" style="font-size:12px;font-weight: 600;">'; bk_error('Error during updating to DB booking filters',__FILE__,__LINE__ ); echo  '</div>';
            }else {
                if ($is_insert) {
                    $newid = (int) $wpdb->insert_id;
                    make_bk_action('added_new_season_filter',$newid);
                }
                echo '<div class="updated ajax_message textleft" style="font-size:12px;font-weight: 600;">'.__('Filter saved' ,'booking').'</div>';
            }
    ...
    function show_booking_date_filter(){ global $wpdb;
        ...
        if (isset($_GET['wpdev_delete'])) {
            ...
            if ($sql_list[0]->count > 0 ) {
                ...
                }else echo '<div class="updated ajax_message textleft" style="font-size:12px;font-weight: 600;">'.__('Filter deleted successfully' ,'booking').'</div>';
            }
            echo '<script type="text/javascript">jQuery(".warning_message").animate({opacity:1},10000).fadeOut(2000);jQuery(".error_message").animate({opacity:1},10000).fadeOut(2000);jQuery(".info_message").animate({opacity:1},5000).fadeOut(2000);</script>';
        }
        ?>
        <div class="create-filter-buttons-section" style="height:40px;">
            <a href="admin.php?page=<?php echo WPDEV_BK_PLUGIN_DIRNAME . '/'. WPDEV_BK_PLUGIN_FILENAME; ?>wpdev-booking-resources&tab=filter&filterdisplay=2"
               style="float: left;  margin:1px 15px 15px 0;font-weight:600;"
        ...
        <div class='meta-box'>  <div  class="postbox" > <h3 class='hndle'><span><?php _e('Listing of season filters' ,'booking'); ?></span></h3> <div class="inside">
            ...
            <table class="booking_table resource_table wpbc_seasonfilters_table" cellpadding="0" cellspacing="0">
                ...
                <?php
                  ...
                  ?>
                      <tr>
                        <td class="wpbc_column_1 <?php echo $td_class; ?>"><span style="font-size:10px;font-weight: 600;"><legend class="wpbc_mobile_legend"><?php _e('ID' ,'booking'); ?>: </legend><?php echo $value->id; ?></span></td>
    ...
    function settings_calendar_unavailable_days() {
        if (isset($_POST['booking_available_days_num_from_today'])) {
            ...
            //FixIn: 6.1.1.18 
            update_bk_option( 'booking_unavailable_extra_in_out', $_POST['booking_unavailable_extra_in_out'] );
            update_bk_option( 'booking_unavailable_extra_minutes_in', $_POST['unavailable_extra_minutes_in'] );
            update_bk_option( 'booking_unavailable_extra_minutes_out', $_POST['unavailable_extra_minutes_out'] );
            update_bk_option( 'booking_unavailable_extra_days_in', $_POST['unavailable_extra_days_in'] );
            update_bk_option( 'booking_unavailable_extra_days_out', $_POST['unavailable_extra_days_out'] );
        ...
        <tr valign="top">
            <th scope="row"><?php _e('Unavailable time before / after booking' ,'booking'); ?>:</th>
            <td>
                <?php $booking_unavailable_extra_in_out = get_bk_option( 'booking_unavailable_extra_in_out'); ?>
                <fieldset>                    
                    <label for="booking_unavailable_extra_in_out_none">
                        <input value="" <?php if ( ($booking_unavailable_extra_in_out == 'none') || (empty($type_of_day_selections)) ) echo 'checked="CHECKED"'; ?>
                            onclick="javascript: jQuery('.booking_unavailable_extra_in_out').addClass('hidden_items');"
                            name="booking_unavailable_extra_in_out" id="booking_unavailable_extra_in_out_none" type="radio" 
                             />
                        <span><?php _e('None' ,'booking'); ?></span>                        
                    </label><br />

                    <label for="booking_unavailable_extra_in_out_minutes">
                        <input value="m" <?php if ($booking_unavailable_extra_in_out == 'm') echo 'checked="CHECKED"'; ?>
                            onclick="javascript: jQuery('.booking_unavailable_extra_in_out').addClass('hidden_items'); jQuery('.booking_unavailable_extra_in_out_minutes').removeClass('hidden_items');"
                            name="booking_unavailable_extra_in_out" id="booking_unavailable_extra_in_out_minutes" type="radio" 
                             />                        
                        <span><?php echo ucfirst(__('minutes' ,'booking')); echo ' / ';  echo ucfirst(__('hours' ,'booking'));?></span>
                    </label><br />

                    <label for="booking_unavailable_extra_in_out_days">
                        <input value="d" <?php if ($booking_unavailable_extra_in_out == 'd')  echo 'checked="CHECKED"'; ?> 
                            onclick="javascript: jQuery('.booking_unavailable_extra_in_out').addClass('hidden_items'); jQuery('.booking_unavailable_extra_in_out_days').removeClass('hidden_items');"
                            name="booking_unavailable_extra_in_out" id="booking_unavailable_extra_in_out_days"  type="radio" 
                             />
                        <span><?php echo ucfirst(__('day(s)' ,'booking'));?></span>
                    </label><br />
                </fieldset>
                <p class="description"><?php 
                                            echo '<strong>' . __('Important!' ,'booking') . '</strong> ' 
                                                 . __('This feature is appling only for bookings for specific timeslots, or if activated check in/out time option.' ,'booking')
                ?></p>                                
            </td>
        </tr>
        
                
        <tr valign="top" class="booking_unavailable_extra_in_out_minutes booking_unavailable_extra_in_out <?php if ($booking_unavailable_extra_in_out != 'm')  echo 'hidden_items'; ?>">
            <?php $unavailable_extra_minutes_in = get_bk_option( 'booking_unavailable_extra_minutes_in'); ?>
            <th scope="row"><label for="unavailable_extra_minutes_in" ><?php _e('Before booking' ,'booking'); ?>:</label></th>
            <td>
                <select id="unavailable_extra_minutes_in" name="unavailable_extra_minutes_in">
                    <option <?php if( empty( $unavailable_extra_minutes_in ) ) echo "selected"; ?> value=""><?php echo ' - '; ?></option>
                    <?php  
                    ...
                    foreach ($extra_time as $extra_value => $extra_label) { ?>
                    <option <?php if($unavailable_extra_minutes_in == $extra_value) echo "selected"; ?> value="<?php echo $extra_value; ?>"><?php echo $extra_label; ?></option>
                    <?php } ?>
                </select>
                <span class="description"><?php _e('Select unavailbale time interval.' ,'booking');?></span>
            </td>
        </tr>
        <tr valign="top" class="booking_unavailable_extra_in_out_minutes booking_unavailable_extra_in_out <?php if ($booking_unavailable_extra_in_out != 'm')  echo 'hidden_items'; ?>">
            <?php $unavailable_extra_minutes_out = get_bk_option( 'booking_unavailable_extra_minutes_out'); ?>
            <th scope="row"><label for="unavailable_extra_minutes_out" ><?php _e('After booking' ,'booking'); ?>:</label></th>
            <td>
                <select id="unavailable_extra_minutes_out" name="unavailable_extra_minutes_out">
                    <option <?php if( empty( $unavailable_extra_minutes_out ) ) echo "selected"; ?> value=""><?php echo ' - '; ?></option>
                    <?php  
                    ...
                    foreach ($extra_time as $extra_value => $extra_label) { ?>
                    <option <?php if($unavailable_extra_minutes_out == $extra_value) echo "selected"; ?> value="<?php echo $extra_value; ?>"><?php echo $extra_label; ?></option>
                    <?php } ?>
                </select>
                <span class="description"><?php _e('Select unavailbale time interval.' ,'booking');?></span>
            </td>
        ...
        <tr valign="top" class="booking_unavailable_extra_in_out_days booking_unavailable_extra_in_out <?php if ($booking_unavailable_extra_in_out != 'd')  echo 'hidden_items'; ?>">
            <?php $unavailable_extra_days_in = get_bk_option( 'booking_unavailable_extra_days_in'); ?>
            <th scope="row"><label for="unavailable_extra_days_in" ><?php _e('Before booking' ,'booking'); ?>:</label></th>
            <td>
                <select id="unavailable_extra_days_in" name="unavailable_extra_days_in">
                    <option <?php if( empty( $unavailable_extra_days_in ) ) echo "selected"; ?> value=""><?php echo ' - '; ?></option>
                    <?php  
                    ...
                    foreach ($extra_time as $extra_value => $extra_label) { ?>
                    <option <?php if($unavailable_extra_days_in == $extra_value) echo "selected"; ?> value="<?php echo $extra_value; ?>"><?php echo $extra_label; ?></option>
                    <?php } ?>
                </select>
                <span class="description"><?php _e('Select unavailbale time interval.' ,'booking');?></span>
            </td>
        ...
        <tr valign="top" class="booking_unavailable_extra_in_out_days booking_unavailable_extra_in_out <?php if ($booking_unavailable_extra_in_out != 'd')  echo 'hidden_items'; ?>">
            <?php $unavailable_extra_days_out = get_bk_option( 'booking_unavailable_extra_days_out'); ?>
            <th scope="row"><label for="unavailable_extra_days_out" ><?php _e('After booking' ,'booking'); ?>:</label></th>
            <td>
                <select id="unavailable_extra_days_out" name="unavailable_extra_days_out">
                    <option <?php if( empty( $unavailable_extra_days_out ) ) echo "selected"; ?> value=""><?php echo ' - '; ?></option>
                    <?php  
                    ...
                    foreach ($extra_time as $extra_value => $extra_label) { ?>
                    <option <?php if($unavailable_extra_days_out == $extra_value) echo "selected"; ?> value="<?php echo $extra_value; ?>"><?php echo $extra_label; ?></option>
                    <?php } ?>
                </select>
                <span class="description"><?php _e('Select unavailbale time interval.' ,'booking');?></span>
            </td>
                      ...
                      foreach ($bk_types as $bt) {
                           ...
                           <tr>
                                ...
                                <td style="<?php if (isset($bt->parent)) if ($bt->parent != 0 ) { echo 'padding-left:50px;'; } ?>" <?php echo $alternative_color; ?> >
                                    ...
                                    <span style="<?php if ( (isset($bt->parent)) && (isset($bt->count)) )  if ( ($bt->parent == 0 ) && ( $bt->count > 1 ) )  { echo 'font-weight: 600;'; }?>"><?php echo $bt->title; ?></span>
                                    <!--input  maxlength="17" type="text"
                                        style="<?php  if ((isset($bt->parent)) && (isset($bt->count)) )    if ( ($bt->parent == 0 ) && ( $bt->count > 1 ) ) { echo 'width:210px;font-weight: 600;'; } else { echo 'width:170px;font-size:11px;'; } ?>"
...
//   A C T I V A T I O N   A N D   D E A C T I V A T I O N    O F   T H I S   P L U G I N  ///////////////////////////////////////////////////
    ...
    function pro_activate() {
            ...
            //FixIn: 6.1.1.18
            add_bk_option( 'booking_unavailable_extra_in_out', '' ); 
            add_bk_option( 'booking_unavailable_extra_minutes_in', '' ); 
            add_bk_option( 'booking_unavailable_extra_minutes_out', '' ); 
            add_bk_option( 'booking_unavailable_extra_days_in', '' ); 
            add_bk_option( 'booking_unavailable_extra_days_out', '' ); 
            //FixIn: 6.1.1.18 End
...
<strong>Details</strong>:<br /><span class="fieldvalue"> [details]</span> 
    ...
    function pro_deactivate(){
        ...
        //FixIn: 6.1.1.18
        delete_bk_option( 'booking_unavailable_extra_in_out' ); 
        delete_bk_option( 'booking_unavailable_extra_minutes_in' ); 
        delete_bk_option( 'booking_unavailable_extra_minutes_out' ); 
        delete_bk_option( 'booking_unavailable_extra_days_in' ); 
        delete_bk_option( 'booking_unavailable_extra_days_out' ); 
        //FixIn: 6.1.1.18 End
