	...
	?>
				...
				<?php
					...
					?>
						...
						<input name="rng_order_<?php echo $attr; ?>" type="hidden" value="<?php echo !empty( $curr_options['wc_settings_prdctfltr_range_filters']['pfr_orderby'][$p] ) ? $curr_options['wc_settings_prdctfltr_range_filters']['pfr_order'][$p] : ''; ?>">
					<?php
					}
						if ( isset($prdctfltr_global['widget_search']) ) {
							$pf_before_title = $before_title . '<span class="prdctfltr_widget_title">';
				...
				<div class="prdctfltr_filter prdctfltr_search" data-filter="pf_search">
					<?php
						...
						if ( $curr_options['wc_settings_prdctfltr_disable_showresults'] == 'no' && ( $curr_styles[5] == 'prdctfltr_disable_bar' || isset( $prdctfltr_global['widget_search'] ) ) && isset($pf_activated['s'] ) ) {
							...
							if ( $paste_title === true ) {
								echo '<a href="#" class="prdctfltr_title_remove" data-key="' . ( isset( $prdctfltr_global['sc_init'] ) ? 'search_products' : 's' ) . '"><i class="prdctfltr-delete"></i></a> <span>'.$pf_activated['s'] . '</span> / ';
							}
				...
				foreach( $pf_activated as $k => $v ) {
					...
					switch( $k ) {
						...
						case 's' :
							...
							if ( isset( $pf_activated['s'] ) && isset( $active_filters['s'] ) ) {
								echo '<span>' . __( 'Search', 'prdctfltr' ) . ': '. $pf_activated['s'] . ' ' . ' <a href="#" class="prdctfltr_title_remove" data-key="' . $k . '"><i class="prdctfltr-delete"></i></a></span>';
							}
						break;
						case 'products_per_page' :
							if ( isset( $pf_activated['products_per_page'] ) ) {
