<?php
/*
Plugin Name: WooCommerce Product Filter - NULL24.NET
	...
	class WC_Prdctfltr {
		...
		function prdctfltr_wc_query( $query ) {
			...
			if ( isset( $pf_activated['orderby'] ) && $pf_activated['orderby'] !== '' ) {
				...
				$make_order = isset( $pf_activated['order'] ) && !empty( $pf_activated['order'] ) ? ( $pf_activated['order'] == 'DESC' ? 'DESC' : 'ASC' ) : ( isset( $orderby_value[1] ) && !empty( $orderby_value[1] ) ? $orderby_value[1] : 'ASC' ) ;
				$order         = $make_order;
