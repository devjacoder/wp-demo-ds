	...
	class WC_Prdctfltr_Shortcodes {
		...
		public static function prdctfltr_respond_550() {
			...
			if ( $query->query_vars['paged'] > 1 ) {
				...
				$filter_query = '?page=' . $query->query_vars['paged'];
			}
			...
			$data['query'] = isset( $filter_query ) ? $filter_query : '';

			if ( get_option( 'permalink_structure' ) !== '' && !isset( $opt['pf_singlesc'] ) ) {

				$current = $wp_query->get_queried_object();

				if ( isset( $current->taxonomy ) ) {

					if ( isset( $curr_filters[$current->taxonomy] ) ) {
						...
						if ( $query->query_vars['paged'] > 1 ) {
							$redirect = $redirect . 'page/' . $query->query_vars['paged'] . '/';
						}
